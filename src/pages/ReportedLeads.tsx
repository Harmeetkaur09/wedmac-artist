...
export default function ReportedLeads() {
  ...
  const getStatusIcon = (status: string) => {
    ...
    if (
      normalize(status).includes("action") ||
      normalize(status).includes("resolved")
    )
  ...
  const underReviewCount = dataSource.filter((it) =>
    normalize(it.status).includes("review")
  ).length;
  const resolvedCount = dataSource.filter((it) =>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Flag className="w-5 h-5 text-primary" /> Reported Leads
            </CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
                      ...
                      <TableCell>
                        {item.proof_documents?.length > 0 ? (
                          <img
                            ...
                            alt={
                              item.proof_documents[0].file_name ||
                              "Proof Document"
                            }
